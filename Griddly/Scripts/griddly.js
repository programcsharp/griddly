...
!function ($)
{
    ...
    var updateFilterDisplayImpl = function (root, filters, options, overrideHasFilter)
    {
        var hasFilter = false;
            ...
            root.find(".griddly-filter-values .filter-display").each(function ()
            {
                ...
                if (!filter.hasClass("griddly-filter-list"))
                {
                    ...
                    if (val != null || valEnd != null)
                    {
                        ...
                        {
                            if (dataType == "String")
                                display += 'contains "' + getFormattedValue(val, dataType, options.currencySymbol) + '"';
                            else
                                display += ' = ' + getFormattedValue(val, dataType, options.currencySymbol);
                        }
                        else if (filter.hasClass("griddly-filter-range"))
                        {
                            if (val != null && valEnd != null)
                                display += 'from ' + getFormattedValue(val, dataType, currencySymbol) + " to " + getFormattedValue(valEnd, dataType, options.currencySymbol);
                            else if (val != null)
                                display += (dataType == "Date" ? "after " : ">= ") + getFormattedValue(val, dataType, options.currencySymbol);
                            else if (valEnd != null)
                                display += (dataType == "Date" ? "before " : "<= ") + getFormattedValue(valEnd, dataType, options.currencySymbol);
                        }
                    }
                ...
                if (displayItems.length)
                {
                    ...
                    for (var i = 0; i < displayItems.length; i++)
                    {
                        ...
                        $(this).append(options.renderFilterDisplay(item.content, item.fieldValue, options.removeIconCssClass) + " ");
                    }
    ...
    Griddly.prototype = {
        ...
        updateFilterDisplay: function ()
        {
            if (this.options.filterMode == "Form")
            {
                ...
                updateFilterDisplayImpl(this.$element, filters, this.options);
            }
            else
            {
                var hasFilter = false;
                ...
                updateFilterDisplayImpl(this.$element, filters, this.options, hasFilter);
            }
